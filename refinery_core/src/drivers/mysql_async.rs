...
async fn query_applied_migrations<'a>(
    transaction: &mut MTransaction<'a>,
    query: &str,
) -> Result<Vec<Migration>, MError> {
    let result = transaction.query_iter(query).await?;
...
impl AsyncTransaction for Pool {
    ...
    async fn execute(&mut self, queries: &[&str]) -> Result<usize, Self::Error> {
        ...
        for query in queries {
            transaction.query_iter(query).await?.drop_result().await?;
            count += 1;
        }
...
impl AsyncQuery<Vec<Migration>> for Pool {
    async fn query(
        ...
        let applied = query_applied_migrations(&mut transaction, query).await?;
        transaction.commit().await?;
        Ok(applied)
    }
}
